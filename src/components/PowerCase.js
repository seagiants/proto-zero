...
const showPowerProps = powerProps => {
  ...
  return props
    .sort(
      (a, b) =>
        a === "enhancement" ? -Infinity : b === "enhancement" ? Infinity : 0
    )
    .map((element, index) => showProp(powerProps, element, index));
  //console.log(props);
};
...
const showProp = (powerProps, prop, index) => {
  const symbol =
    propsSymbols[prop] !== undefined
      ? propsSymbols[prop](
          powerSize.width * 0.2,
          powerSize.height * (0.45 + index * 0.15),
          0.03,
          powerProps[prop]
        )
      : null;
  return <g key={prop}>{symbol}</g>;
};
  ...
  return (
    <div>
      ...
      >
        ...
        {/*The cost symbol*/}
        {symbols.cost(
          powerSize.x + powerSize.width * 0.85,
          powerSize.y + powerSize.height * 0.75,
          1,
          "white"
        )}
        {/*The head symbol*/}
        {symbols[activePower.symbol](
          symbolSize[activePower.symbol].x(powerSize),
          symbolSize[activePower.symbol].y(powerSize),
          symbolSize[activePower.symbol].scale(powerSize),
          "black"
        )}
        {/*Tests*/}
